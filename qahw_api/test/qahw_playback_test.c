...
#define ALAC_KVPAIR                             \
    "music_offload_alac_avg_bit_rate=%d;"       \
...
static ssize_t get_bytes_to_read(FILE *file, int file_type) {
  ...
  if (file_type == FILE_DTS) {
    ...
    while (!feof(file) && (header_read_size < 1024) &
                              (fread(&keyword, sizeof(char), DTSHD_META_KEYWORD_SIZE, file) == DTSHD_META_KEYWORD_SIZE)) {
    ...
    while (!feof(file) &&
           fread(&keyword, sizeof(uint8_t), DTSHD_META_KEYWORD_SIZE, file) == DTSHD_META_KEYWORD_SIZE) {
