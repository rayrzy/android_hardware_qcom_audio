...
void audio_extn_fm_set_parameters(struct audio_device *adev,
  ...
  if (ret >= 0) {
    ...
    if (val != 0) {
      if (val & AUDIO_DEVICE_OUT_FM && fmmod.is_fm_running == false) {
        audio_devices_t OutputDevice = val & ~AUDIO_DEVICE_OUT_FM;
        fm_start(adev, OutputDevice);
      } else if (!(val & AUDIO_DEVICE_OUT_FM) && fmmod.is_fm_running == true) {
  ...
  ret = str_parms_get_str(parms, AUDIO_PARAMETER_KEY_REC_PLAY_CONC,
                          value, sizeof(value));
  if ((ret >= 0) && (fmmod.is_fm_running == true)) {
    ...
    list_for_each(node, &adev->usecase_list) {
      ...
      if (usecase->id == USECASE_AUDIO_PLAYBACK_FM) {
        if (fmmod.fm_device != device) {
          ALOGV(
              "%s selected routing device %x current device %x"
              "are different, reroute on selected device",
              __func__,
