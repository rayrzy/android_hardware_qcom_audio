...
static void audio_extn_ext_disp_set_parameters(const struct audio_device *adev,
  ...
  if (str_parms_get_str(parms, "connect", value, sizeof(value)) >= 0 && (atoi(value) & AUDIO_DEVICE_OUT_AUX_DIGITAL)) {
  ...
  } else if (str_parms_get_str(parms, "disconnect", value, sizeof(value)) >= 0 && (atoi(value) & AUDIO_DEVICE_OUT_AUX_DIGITAL)) {
...
bool audio_extn_should_use_handset_anc(int in_channels) {
  ...
  return (aextnmod.aanc_enabled && aextnmod.anc_enabled && (in_channels == 1));
}
...
int audio_extn_parse_compress_metadata(struct stream_out *out,
  ...
  if (out->format == AUDIO_FORMAT_FLAC) {
    ...
    ALOGV(
        "ALAC CSD values: frameLength %d bitDepth %d numChannels %d"
    ...
    ALOGV(
        "APE CSD values: compatibleVersion %d compressionLevel %d"
    ...
    ALOGV(
        "WMA params: fmt %x, bit rate %x, balgn %x, sr %d, chmsk %x"
        " encop %x, op1 %x, op2 %x",
...
int audio_extn_set_device_cfg_params(struct audio_device *adev,
  ...
  ALOGV(
      "%s:: Device Config Params from Client samplerate %d  channels %d"
