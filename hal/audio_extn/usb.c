...
static int usb_get_capability(int type,
  ...
  ALOGV("%s: for %s", __func__, (type == USB_PLAYBACK) ? PLAYBACK_PROFILE_STR : CAPTURE_PROFILE_STR);
  ...
  str_start = strstr(read_buf, ((type == USB_PLAYBACK) ? PLAYBACK_PROFILE_STR : CAPTURE_PROFILE_STR));
  if (str_start == NULL) {
    ALOGE("%s: error %s section not found in usb config file",
          __func__, ((type == USB_PLAYBACK) ? PLAYBACK_PROFILE_STR : CAPTURE_PROFILE_STR));
    ret = -EINVAL;
    goto done;
  }
  str_end = strstr(read_buf, ((type == USB_PLAYBACK) ? CAPTURE_PROFILE_STR : PLAYBACK_PROFILE_STR));
...
unsigned long audio_extn_usb_find_service_interval(bool min,
  ...
  list_for_each(node_i, &usbmod->usb_card_conf_list) {
    ...
    list_for_each(node_j, &card_info->usb_device_conf_list) {
      ...
      if (match) {
        interval_us = min ? _MIN(interval_us, dev_info->service_interval_us) : _MAX(interval_us, dev_info->service_interval_us);
      }
    }
    break;
  }
        ...
        switch (usecase->id) {
          ...
          case USECASE_AUDIO_PLAYBACK_ULL: {
            if (uc_info != usecase) {
              ...
              ALOGV(
                  "%s:current ULL uc is the same as incoming uc_info \
                                   which means we are stopping the output stream, \
                                   we don't want to set burst mode to false",
                  __func__);
            }
            break;
          }
