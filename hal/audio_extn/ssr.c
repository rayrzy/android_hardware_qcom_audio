...
static int32_t ssr_init_surround_sound_3mic_lib(unsigned long buffersize, int num_in_chan, int num_out_chan, int sample_rate) {
  ...
  if (!ssrmod.surround_raw_buffer) {
    ALOGE(
        "%s: Memory allocation failure. Not able to allocate "
        "memory for surroundInputBuffer",
        __func__);
    ret = -ENOMEM;
    goto init_fail;
  }
  ...
  *update_params = false;
  ...
  if (audio_extn_ssr_check_usecase(in)) {
    if (representation == AUDIO_CHANNEL_REPRESENTATION_INDEX) {
       ...
       */
      ALOGD("%s: SSR supports only channel representation position, channel_mask(%#x)", __func__, config->channel_mask);
...
int32_t audio_extn_ssr_init(struct stream_in *in, int num_out_chan) {
  ...
  if (ssrmod.ssr_3mic != 0) {
    ...
    if (0 != ret) {
      ALOGE(
          "%s: ssr_init_surround_sound_3mic_lib failed: %d  "
          "buffer_size:%d",
          __func__, ret, buffer_size);
      goto fail;
    }
  }
