...
static snd_device_t fixup_usb_headset_mic_snd_device(void *platform,
  ...
  if (audio_extn_usb_is_config_supported(&bit_width, &sr, &ch, false) && ((ch == 6) || (ch == 8))) {
    return other;
  } else {
    return base;
  }
...
snd_device_t platform_get_output_snd_device(void *platform, struct stream_out *out) {
  ...
  if (devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE ||
      devices & AUDIO_DEVICE_OUT_WIRED_HEADSET ||
      devices & AUDIO_DEVICE_OUT_LINE) {
    if (devices & AUDIO_DEVICE_OUT_WIRED_HEADSET && audio_extn_get_anc_enabled()) {
      if (audio_extn_should_use_fb_anc())
  ...
  if (in_device != AUDIO_DEVICE_NONE &&
    ...
    if (out_device & AUDIO_DEVICE_OUT_EARPIECE) {
      ...
      if ((my_data->source_mic_type & SOURCE_DUAL_MIC) &&
        ...
        ALOGE(
            "%s: something wrong: source type (%d) channel_count (%d) .."
            " no combination found .. setting to mono",
            __func__,
...
static bool platform_check_capture_codec_backend_cfg(struct audio_device *adev,
  ...
  if ((voice_is_in_call(adev) || adev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
    ALOGW(
        "%s:txbecf: afe: Use default bw and sr for voice/voip calls and "
        "for unprocessed/camera source",
        __func__);
    bit_width = CODEC_BACKEND_DEFAULT_BIT_WIDTH;
    sample_rate = CODEC_BACKEND_DEFAULT_SAMPLE_RATE;
