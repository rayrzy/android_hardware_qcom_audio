...
static bool may_use_noirq_mode(struct audio_device *adev, audio_usecase_t uc_id,
  ...
  int dev_id = platform_get_pcm_device_id(uc_id, dir == 0 ? PCM_PLAYBACK : PCM_CAPTURE);
...
static int parse_snd_card_status(struct str_parms *parms, int *card,
  ...
  *status = !strcmp(state, "ONLINE") ? CARD_STATUS_ONLINE : CARD_STATUS_OFFLINE;
  return 0;
}
...
__attribute__((visibility("default"))) bool audio_hw_send_gain_dep_calibration(int level) {
  bool ret_val = false;
...
__attribute__((visibility("default"))) int audio_hw_get_gain_level_mapping(struct amp_db_and_gain_table *mapping_tbl,
                                                                           int table_size) {
  int ret_val = 0;
...
static void check_usecases_codec_backend(struct audio_device *adev,
  ...
  list_for_each(node, &adev->usecase_list) {
    ...
    ALOGD(
        "%s:becf: (%d) check_usecases curr device: %s, usecase device:%s "
        "backends match %d",
        __func__, i,
...
int select_devices(struct audio_device *adev, audio_usecase_t uc_id) {
  ...
  if (is_btsco_device(out_snd_device, in_snd_device) || is_a2dp_device(out_snd_device)) {
    if (usecase->type == VOIP_CALL) {
      ...
      if ((usecase->stream.out != NULL) && (usecase->stream.out != adev->primary_output) && usecase->stream.out->started) {
        if (is_bt_soc_on(adev) == false) {
...
static uint64_t get_actual_pcm_frames_rendered(struct stream_out *out, struct timespec *timestamp) {
  ...
  ALOGVV(
      "%s signed frames %lld out_written %lld kernel_buffer_size %d"
      "bytes/sample %zu channel count %d",
      __func__, (long long int)signed_frames,
  ...
  if (out->hal_output_suspend_supported && out->dynamic_pm_qos_config_supported) {
    ...
    if (str_parms_get_str(parms, "suspend_playback", value, sizeof(value)) >= 0) {
      ...
      if (!strncmp(value, "false", 5)) {
        ...
        ALOGE(
            "%s: Wrong value sent for suspend_playback, expected true/false,"
            " got %s",
            __func__, value);
        ret = -1;
      }
  ...
  if (audio_extn_cin_attached_usecase(in->usecase)) {
    ...
    if (audio_extn_ssr_get_stream() == in) {
      ...
      if (!ret && bytes > 0 && (in->format == AUDIO_FORMAT_PCM_8_24_BIT)) {
        if (audio_extn_utils_convert_format_24_8_to_8_24(buffer, bytes) != bytes) {
          ret = -EINVAL;
          goto exit;
        }
      } else if (ret < 0) {
...
int adev_open_output_stream(struct audio_hw_device *dev,
  ...
  *stream_out = NULL;
  ...
  ALOGD(
      "%s: enter: format(%#x) sample_rate(%d) channel_mask(%#x) devices(%#x) flags(%#x)\
        stream_handle(%p)",
...
static int adev_update_voice_comm_input_stream(struct stream_in *in,
  ...
  if (valid_rate && valid_ch &&
    ...
    in->config.period_size = VOIP_IO_BUF_SIZE(in->sample_rate,
                                              ...
                                              DEFAULT_VOIP_BIT_DEPTH_BYTE) /
                             2;
  } else {
    ALOGW(
        "%s No valid input in voip, use defaults"
...
static int adev_open_input_stream(struct audio_hw_device *dev,
  ...
  *stream_in = NULL;
  ...
  ALOGD(
      "%s: enter: sample_rate(%d) channel_mask(%#x) devices(%#x)\
        stream_handle(%p) io_handle(%d) source(%d) format %x",
  ...
  if (is_usb_dev && may_use_hifi_record) {
    ...
    config->format = AUDIO_FORMAT_PCM_16_BIT;
  } else if (property_get_bool("vendor.audio.capture.pcm.32bit.enable", false) && config->format == AUDIO_FORMAT_PCM_32_BIT) {
