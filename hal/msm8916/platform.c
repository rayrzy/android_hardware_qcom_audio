...
static int init_be_dai_name_table(struct audio_device *adev) {
  ...
  for (i = 0; i < SND_DEVICE_MAX; i++) {
    ...
    for (j = 0; j < max_be_dai_names; j++) {
      if (strcmp(hw_interface_table[i], be_dai_name_table[j].be_name) == 0) {
...
acdb_init_fail:
  ...
  ALOGD(
      "%s: Fluence_Type(%d) max_mic_count(%d) mic_type(0x%x) fluence_in_voice_call(%d)"
...
int platform_set_native_support(int na_mode) {
  if (NATIVE_AUDIO_MODE_SRC == na_mode || NATIVE_AUDIO_MODE_TRUE_44_1 == na_mode || NATIVE_AUDIO_MODE_MULTIPLE_44_1 == na_mode) {
    ...
    ALOGD("%s:napb: native audio playback enabled in (%s) mode", __func__,
          ((na_mode == NATIVE_AUDIO_MODE_SRC) ? "SRC" : (na_mode == NATIVE_AUDIO_MODE_TRUE_44_1) ? "True"
                                                                                                 : "Multiple"));
  } else {
    ...
    if (na_props.platform_na_prop_enabled) {
      ...
      list_for_each(node, &(platform->adev)->usecase_list) {
        ...
        if (is_offload_usecase(usecase->id) &&
          ...
          ALOGD(
              "%s:napb: triggering dynamic device switch for usecase(%d: %s)"
              " stream(%p), device(%d)",
              __func__, usecase->id,
...
snd_device_t platform_get_output_snd_device(void *platform, struct stream_out *out) {
  ...
  audio_channel_mask_t channel_mask = (adev->active_input == NULL) ? AUDIO_CHANNEL_IN_MONO : adev->active_input->channel_mask;
  ...
  if (popcount(devices) == 2) {
    ...
    if (devices == (AUDIO_DEVICE_OUT_WIRED_HEADPHONE |
      ...
      if (my_data->is_wsa_speaker)
        snd_device = adev->bt_wb_speech_enabled ? SND_DEVICE_OUT_SPEAKER_WSA_AND_BT_SCO_WB : SND_DEVICE_OUT_SPEAKER_WSA_AND_BT_SCO;
      else
        snd_device = adev->bt_wb_speech_enabled ? SND_DEVICE_OUT_SPEAKER_AND_BT_SCO_WB : SND_DEVICE_OUT_SPEAKER_AND_BT_SCO;
    } else {
  ...
  if ((mode == AUDIO_MODE_IN_CALL) ||
    ...
    if (devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE ||
      ...
      if (snd_device == SND_DEVICE_NONE) {
        snd_device = audio_extn_usb_is_capture_supported() ? SND_DEVICE_OUT_VOICE_USB_HEADSET : SND_DEVICE_OUT_VOICE_USB_HEADPHONES;
      }
    } else if (devices & AUDIO_DEVICE_OUT_ALL_SCO) {
  ...
  if (devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE ||
    ...
    } else if (devices & AUDIO_DEVICE_OUT_WIRED_HEADSET && audio_extn_get_anc_enabled()) {
#ifdef RECORD_PLAY_CONCURRENCY
...
snd_device_t platform_get_input_snd_device(void *platform, audio_devices_t out_device) {
   ...
   */
  audio_source_t source = (adev->active_input == NULL) ? AUDIO_SOURCE_DEFAULT : adev->active_input->source;

  audio_mode_t mode = adev->mode;
  audio_devices_t in_device = ((adev->active_input == NULL) ? AUDIO_DEVICE_NONE : adev->active_input->device) & ~AUDIO_DEVICE_BIT_IN;
  audio_channel_mask_t channel_mask = (adev->active_input == NULL) ? AUDIO_CHANNEL_IN_MONO : adev->active_input->channel_mask;
  snd_device_t snd_device = SND_DEVICE_NONE;
  int channel_count = popcount(channel_mask);
  int str_bitwidth = (adev->active_input == NULL) ? CODEC_BACKEND_DEFAULT_BIT_WIDTH : adev->active_input->bit_width;
  ...
  if ((out_device != AUDIO_DEVICE_NONE) && ((mode == AUDIO_MODE_IN_CALL) ||
    ...
    in_device = ((out_device == AUDIO_DEVICE_NONE) ? AUDIO_DEVICE_IN_BUILTIN_MIC : in_device) & ~AUDIO_DEVICE_BIT_IN;
...
int platform_set_hdmi_channels(void *platform, int channel_count) {
  ...
  switch (channel_count) {
    case 8:
      channel_cnt_str = "Eight";
      break;
    case 7:
      channel_cnt_str = "Seven";
      break;
    case 6:
      channel_cnt_str = "Six";
      break;
    case 5:
      channel_cnt_str = "Five";
      break;
    case 4:
      channel_cnt_str = "Four";
      break;
    case 3:
      channel_cnt_str = "Three";
      break;
    default:
      channel_cnt_str = "Two";
      break;
...
static int platform_set_codec_backend_cfg(struct audio_device *adev,
  ...
  if (adev_device_cfg_ptr->use_client_dev_cfg) {
    ALOGV(
        "%s::: Updating with the config set by client "
...
int platform_get_codec_backend_cfg(struct audio_device *adev,
  ...
  ALOGV(
      "%s:becf: afe: bitwidth %d, samplerate %d channels %d format %d"
      ", backend_idx %d device (%s)",
...
static void platform_check_hdmi_backend_cfg(struct audio_device *adev,
  ...
  ALOGI(
      "%s:becf: HDMI: bitwidth %d, samplerate %d, channels %d"
      ", usecase = %d",
      __func__, bit_width,
      sample_rate, channels, usecase->id);

  if (audio_extn_passthru_is_enabled() && audio_extn_passthru_is_active() && (usecase->stream.out->compr_config.codec->compr_passthr != 0)) {
  ...
  ALOGI(
      "%s:becf: afe: HDMI backend: passthrough %d updated bit width: %d and sample rate: %d"
      "channels %d",
      __func__, passthrough_enabled, bit_width,
...
static bool platform_check_codec_backend_cfg(struct audio_device *adev,
  ...
  ALOGI(
      "%s:becf: afe: Codec selected backend: %d current bit width: %d sample rate: %d channels: %d "
      "usecase %d device (%s)",
  ...
  if (!voice_call_active && codec_device_supports_native_playback(usecase->devices)) {
    if (audio_is_true_native_stream_active(adev)) {
      if (check_hdset_combo_device(snd_device)) {
        ...
        sample_rate = CODEC_BACKEND_DEFAULT_SAMPLE_RATE;
        bit_width = CODEC_BACKEND_DEFAULT_BIT_WIDTH;
        ALOGD("%s:becf: afe: port to run at 48k if combo device or in voice call", __func__);
      } else {
        /*
  ...
  if (!codec_device_supports_native_playback(usecase->devices) &&
    ...
    if (bit_width >= 24) {
      ...
      ALOGD(
          "%s:becf: afe: reset bitwidth to %d (based on supported"
          " value for this platform)",
          __func__, bit_width);
    }

    ALOGD(
        "%s:becf: afe: playback on codec device not supporting native playback set "
        "default Sample Rate(48k)",
        __func__);
  }
  ...
  if (!my_data->hifi_audio && (usecase->devices & AUDIO_DEVICE_OUT_ALL_CODEC_BACKEND)) {
    ALOGD(
        "%s:becf: afe: only 48KHZ sample rate is supported "
        "Configure afe to default Sample Rate(48k)",
        __func__);
    sample_rate = CODEC_BACKEND_DEFAULT_SAMPLE_RATE;
  }
  ...
  ALOGI(
      "%s:becf: afe: Codec selected backend: %d updated bit width: %d "
      "sample rate: %d channels: %d",
      __func__, backend_idx,
  ...
  if ((bit_width != my_data->current_backend_cfg[backend_idx].bit_width) ||
    ...
    backend_change = true;
    ALOGI(
        "%s:becf: afe: Codec backend needs to be updated. new bit width: %d"
  ...
  if (adev_device_cfg_ptr->use_client_dev_cfg) {
    ALOGV(
        "%s: Codec backend needs to be updated as Client provided "
        "config params",
        __func__);
    backend_change = true;
  }
...
bool platform_check_and_set_codec_backend_cfg(struct audio_device *adev,
  ...
  ALOGI(
      "%s:becf: afe: bitwidth %d, samplerate %d channels %d"
      ", backend_idx %d usecase = %d device (%s)",
      __func__, backend_cfg.bit_width,
...
static bool platform_check_capture_codec_backend_cfg(struct audio_device *adev,
  ...
  ALOGI(
      "%s:txbecf: afe: Codec selected backend: %d current bit width: %d and "
      "sample rate: %d, channels %d format %d",
      __func__, backend_idx, bit_width,
  ...
  ALOGI(
      "%s:txbecf: afe: Codec selected backend: %d updated bit width: %d and "
      "sample rate: %d",
  ...
  if ((bit_width != my_data->current_backend_cfg[backend_idx].bit_width) ||
    ...
    backend_change = true;
    ALOGI(
        "%s:txbecf: afe: Codec backend needs to be updated. new bit width: %d "
...
bool platform_check_and_set_capture_codec_backend_cfg(struct audio_device *adev,
  ...
  ALOGI(
      "%s:txbecf: afe: bitwidth %d, samplerate %d, channel %d format %d"
      ", backend_idx %d usecase = %d device (%s)",
      __func__,
...
int ramp_speaker_gain(struct audio_device *adev, bool ramp_up, int target_ramp_up_gain) {
  ...
  if (!ctl_left || !ctl_right) {
    ...
    return -EINVAL;
  } else if ((mixer_ctl_get_num_values(ctl_left) != 1) || (mixer_ctl_get_num_values(ctl_right) != 1)) {
