...
__attribute__((visibility("default"))) int visualizer_hal_start_output(audio_io_handle_t output, int pcm_id) {
  int ret = 0;
  struct listnode *node;
...
__attribute__((visibility("default"))) int visualizer_hal_stop_output(audio_io_handle_t output, int pcm_id) {
  int ret = 0;
  struct listnode *node;
  struct listnode *fx_node;
    ...
    case VISUALIZER_CMD_MEASURE: {
      if (pReplyData == NULL || replySize == NULL ||
        ...
        if (replySize == NULL) {
          ...
          ALOGV(
              "%s VISUALIZER_CMD_MEASURE error *replySize %u <"
